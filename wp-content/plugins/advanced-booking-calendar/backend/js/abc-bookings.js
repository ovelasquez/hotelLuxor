...
jQuery('.abc-availability-table-button-right').live('click', function(){
...
jQuery('.abc-availability-table-button-left').live('click', function(){
	var startdate = new Date(+new Date(jQuery(this).data('startdate')) - 12096e5); // - 14 days
	abcTriggerButtons(true);
	data = {
			action: 'abc_booking_getAvailabilityTable',
...
jQuery('.abcMonthSelector').live('click', function(){
...
jQuery('.abcYearSelector').live('click', function(){
	var startdate = jQuery(this).data('startdate');
	abcTriggerButtons(true);
	data = {
			action: 'abc_booking_getAvailabilityTable',
...
jQuery('#abcBookingSearchButton').live('click', function(){
...
jQuery( "input[name=abc-extras-checkbox]").live('click', function () {
...
function abc_checkDates() {
	...
	if( jQuery("#start").val() && jQuery("#end").val() && jQuery( "select[name='calendar_id']").val() ){
	    ...
	    jQuery.when( q1, q2, q3 ).done( function(){abc_calculatePrice();});
		return false;
	}	
}
