...
function abc_booking_addCalendar() {
    ...
    if (isset($_POST["name"]) && isset($_POST["maxAvailabilities"]) && isset($_POST["maxUnits"]) && isset($_POST["page_id"]) && intval($_POST["minimumStayPreset"]) > 0) {
        ...
        if ($rooms + $maxAvailabilities <= 50) {
            ...
            for ($i = 1; $i <= $maxAvailabilities; $i++) {
                $wpdb->insert($wpdb->prefix . 'advanced_booking_calendar_rooms', array('calendar_id' => $calendarId,
                    'name' => $name . '-' . $i
                ));
            }
        } else {
...
function abc_booking_editCalendar() {
    ...
    if (isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["maxAvailabilities"]) && isset($_POST["maxUnits"]) && isset($_POST["page_id"]) && intval($_POST["minimumStayPreset"]) > 0) {
        $er = $wpdb->get_row('SELECT SUM(maxAvailabilities) as rooms FROM ' . $wpdb->prefix . 'advanced_booking_calendar_calendars', ARRAY_A);
        $rooms = $er["rooms"];
        ...
        $name = sanitize_text_field($_POST["name"]);
        $maxUnits = intval($_POST["maxUnits"]);
        $pricePreset = intval($_POST["pricePreset"]);
        $minimumStayPreset = intval($_POST["minimumStayPreset"]);
        $partlyBooked = 1;
        if (isset($_POST['partlyBooked']) && intval($_POST['partlyBooked']) > 0 && intval($_POST['partlyBooked']) < $maxAvailabilities) {
            $partlyBooked = intval($_POST['partlyBooked']);
        ...
        $infoPage = intval($_POST["page_id"]);
        $infoText = implode("\n", array_map('sanitize_text_field', explode("\n", $_POST["infotext"])));
        if ($rooms + $maxAvailabilities <= 50 && $er['maxAvailabilities'] <= intval($_POST["maxAvailabilities"])) {
            $wpdb->update($wpdb->prefix . 'advanced_booking_calendar_calendars', array(
                'name' => $name,
                'maxUnits' => $maxUnits,
                'maxAvailabilities' => $maxAvailabilities,
                'pricePreset' => $pricePreset,
        ...
        if ($er['maxAvailabilities'] < $maxAvailabilities) { // Adding new rooms when room number is raised
            for ($i = $er["maxAvailabilities"]; $i < $maxAvailabilities; $i++) {
                $wpdb->insert($wpdb->prefix . 'advanced_booking_calendar_rooms', array('calendar_id' => $er['id'],
                    'name' => $er['name'] . '-' . ($i + 1)));
            }
        }
...
function abc_booking_editSeason() {
    ...
    if (isset($_POST["id"]) && isset($_POST["title"]) && isset($_POST["price"]) && intval($_POST["minimumStay"]) >= 1) {
        $wpdb->update($wpdb->prefix . 'advanced_booking_calendar_seasons', array(
            'title' => sanitize_text_field($_POST["title"]),
            'price' => sanitize_text_field($_POST["price"]),
            'minimumStay' => intval($_POST["minimumStay"])), array('id' => intval($_POST["id"])));
    }
...
function abc_booking_editRoomNames() {
    ...
    if (isset($_POST["name-0"]) && isset($_POST["calendarId"])) {
        ...
        for ($i = 0; $i <= ($maxAvailabilities - 1); $i++) {
            $wpdb->update($wpdb->prefix . 'advanced_booking_calendar_rooms', array('name' => sanitize_text_field($_POST["name-" . $i])), array('id' => sanitize_text_field($_POST["id-" . $i])));
        }
    }
...
function abc_booking_editSeasonAssignment() {
    ...
    if (isset($_POST["id"]) && isset($_POST["calendar"]) && isset($_POST["season"]) && isset($_POST["dateStart"]) && isset($_POST["dateEnd"])) {
        ...
        $row = $wpdb->get_results("SELECT COUNT(*) as co FROM " . $wpdb->prefix . "advanced_booking_calendar_seasons_assignment
				 				   WHERE calendar_id = '" . sanitize_text_field($_POST["calendar"]) . "' AND
				 				   		id != '" . intval($_POST["id"]) . "' AND 
										 (start < '" . $normToValue . "' AND
										  end > '" . $normFromValue . "')", ARRAY_A);
        if ($row[0]["co"] > 0) {
            wp_die("Overlap", "Seasons assignment", array('back_link' => true));
        }
        ...
        switch ($_GET["setting"]) {
            case 'roomLimit':
                $notices .= '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
										<p><strong>' . __('Number of rooms is limited to 50 rooms! If you need more rooms, please download our <a href="https://booking-calendar-plugin.com/pro-download/" target="_blank">Pro-Version</a>.', 'advanced-booking-calendar') . '</strong></p><button type="button" class="notice-dismiss">
            ...
            if ($row["co"] == 0) {
                ...
                $output = '<div class="wrap">
					  ...
					  <tr>
						...
						<td align="left"><input placeholder="90,00" name="pricePreset" id="pricePreset" type="number" value="' . $row[0]["pricePreset"] . '" class="regular-text code" min="1" required />
					  ...
					  <div class="wrap">
						...
						<table class="form-table">
						  ...
						  <tr>
							...
							<td align="left"><input name="price" placeholder="50" id="price" value="' . $row[0]["price"] . '" type="number" min="0" class="regular-text code" required />
            ...
            $output .= '<li>
				  ...
				  <div class="wrap">
					...
					<table class="form-table">
					  ...
					  <tr>
						<td><label for="pricePreset">' . __('Price Preset', 'advanced-booking-calendar') . '</label></td>
						<td align="left"><input placeholder="90,00" name="pricePreset" id="pricePreset" type="number" class="regular-text code" min="1" required />
											 <p class="description">' . __('If no season is active, the price preset will be used for price calculations', 'advanced-booking-calendar') . '</p></td>
					  </tr>
					  <tr>
						<td><label for="minimumStayPreset">' . __('Minimum Stay Preset', 'advanced-booking-calendar') . '</label></td>
            ...
            $output .= '<li>
					  ...
					  <div class="wrap">
						...
						<table class="form-table">
						  ...
						  <tr>
							<td><label for="price">' . __('Price', 'advanced-booking-calendar') . '</label></td>
							<td align="left"><input name="price" placeholder="50" id="price" type="number" min="0" class="regular-text code" required />
											 <p class="description">' . __('The currency code will be attached automatically.', 'advanced-booking-calendar') . '</p></td>
						  </tr>
						  <tr>
							<td><label for="minimumStay">' . __('Minimum Stay', 'advanced-booking-calendar') . '</label></td>
