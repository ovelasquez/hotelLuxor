...
function abc_booking_showSingleCalendar($atts) {
    if (!isset($atts['calendar'])) {
        return '<p>Calendar ID not set. Please check the shortcode.</p>';
    } else {
        global $abcUrl;
...
function ajax_abc_booking_setDataRange() {
    ...
    if (!isset($_POST['start']) OR ! isset($_POST['end'])) {
        ...
        if ($start != 0) {
            $output .= '<div class="abc-column"><b>' . __('Checkin', 'advanced-booking-calendar') . ':</b> ' . date($dateformat, $start) . '<br/>
				<b>' . __('Checkout', 'advanced-booking-calendar') . ':</b> ';
            if ($end != 0 && $end > $start) {
                $success = true;
                $output .= date($dateformat, $end);
                $numberOfDays = abc_booking_dateDiffInDays($end, $start);
                $output .= '<br/><b>' . __('Total price', 'advanced-booking-calendar') . ': </b>
                    ...
                    $output .='</div>
						<div class="abc-column">
							...
							<button class="abc-submit">
								<span class="abc-submit-text">' . __('Book now', 'advanced-booking-calendar') . '</span>
							</button>
							';
                }
                ...
                $output .= '<input type="hidden" name="abc-calendar" value="' . $calendarId . '">';
                $output .= '<input type="hidden" name="abc-trigger" value="1">';
                $output .= '</form>';
            } else {
                $output .= '-';
            }
